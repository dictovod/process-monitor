...
# ─────────────────────────────────────────────
TELEGRAM_TOKEN = "5214842839:AAGLsMzJh_T0g5iih3Q7tSXbBbX_AP4bwF8"
                ...
                for cid in list(pending.keys()):
                    ...
                    if len(procs) == 1:
                        ...
                        for proc in procs:
                            ...
                            txt = (
                                "<b>" + n + "</b>" + cnt_str + "\n"
                                "CPU " + str(round(proc["cpu"], 1)) + "%  "
                                "RAM " + str(proc["memory_mb"]) + " MB  "
                                + proc["username"]
                            )
                            send_message(cid, txt, markup=kb_process_compact(n))
                    pending[cid].clear()
        except Exception as e:
